#!/usr/bin/env python3
"""Compare patch uniqueness confidence heatmaps for LIBERO demos."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Patch uniqueness confidence heatmap visualization")
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Goal frame index (supports negative indexing, default: -1 for final frame).",
    )
    parser.add_argument(
        "--max_frames",
        type=int,
        default=None,
        help="Optional cap on rendered frames (for quick experiments).",
    )
    parser.add_argument(
        "--video_name",
        type=str,
        default="uniqueness_compare.mp4",
...
def main():
    ...
    goal_frame = int(args.goal_frame)
    if goal_frame < 0:
        goal_frame = len(frames) + goal_frame
    if goal_frame < 0 or goal_frame >= len(frames):
        raise IndexError(f"goal_frame {args.goal_frame} out of range for {len(frames)} frames")
    ...
    goal_cam1 = to_uint8_rgb(frames[goal_frame]["cam1"])
    goal_cam2 = to_uint8_rgb(frames[goal_frame]["cam2"])
    ...
    print("[Info] rendered_num_frames:", num_frames)
    print("[Info] goal_frame:", goal_frame)
    ...
    for t in tqdm(range(num_frames), desc="Rendering uniqueness heatmaps"):
