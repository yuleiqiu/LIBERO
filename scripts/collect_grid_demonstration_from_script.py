...
def gather_demonstrations_as_hdf5(
    directory, out_dir, env_info, problem_info, args, remove_directory=[]
):
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=
        """
        Collect demonstration in the environment defined by provided BDDL file
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the designated BDDL file"
    )
    ...
    new_dir = os.path.join(
        ...
        + args.bddl_file.split("/")[-1].split(".")[0],
    )
    ...
    while i < args.num_demonstration:
        if timeout_count > 4:
            ...
            shutil.rmtree(tmp_directory)  # Clean up temporary directory
            shutil.rmtree(new_dir)  # Remove the directory if it was not saved
            break
        ...
        if saving:
            gather_demonstrations_as_hdf5(
                tmp_directory, new_dir, env_info, problem_info, args, remove_directory
            )
            i += 1
